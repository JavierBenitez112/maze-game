...
impl MenuRenderer {
    ...
    pub fn preload_textures(&mut self, window: &mut RaylibHandle, raylib_thread: &RaylibThread, menu: &Menu) {
        // Precargar imagen de fondo
        if let Some(ref bg_path) = menu.background_image {
            // Ignorar paths vacíos
            if !bg_path.is_empty() && !self.texture_cache.contains_key(bg_path) {
                match window.load_texture(&raylib_thread, bg_path) {
                    Ok(texture) => {
                        println!("Precargando textura de fondo: {}", bg_path);
                        self.texture_cache.insert(bg_path.clone(), texture);
                    }
                    Err(e) => {
                        println!("Error precargando textura de fondo {}: {:?}", bg_path, e);
                    }
                }
            }
        }

        // Precargar imágenes de opciones
        for option in &menu.options {
            if let Some(ref image_path) = option.image_path {
                // Ignorar paths vacíos
                if !image_path.is_empty() && !self.texture_cache.contains_key(image_path) {
                    match window.load_texture(&raylib_thread, image_path) {
                        Ok(texture) => {
                            println!("Precargando textura de opción: {}", image_path);
                            self.texture_cache.insert(image_path.clone(), texture);
                        }
                        Err(e) => {
                            println!("Error precargando textura de opción {}: {:?}", image_path, e);
                        }
                    }
                }
            }
    ...
    pub fn render_menu(&mut self, window: &mut RaylibHandle, raylib_thread: &RaylibThread, menu: &Menu) {
        ...
        // SIEMPRE limpiar el fondo primero para evitar rastros
        d.clear_background(Color::BLACK);

        // Dibujar imagen de fondo si existe y no está vacía
        if let Some(ref bg_path) = menu.background_image {
            if !bg_path.is_empty() {
                if let Some(bg_texture) = self.texture_cache.get(bg_path) {
                    ...
                    d.draw_texture_pro(
                        bg_texture,
                        Rectangle::new(0.0, 0.0, bg_width, bg_height),
                        Rectangle::new(x, y, scaled_width, scaled_height),
                        Vector2::new(0.0, 0.0),
                        0.0,
                        Color::WHITE,
                    );
                } else {
                    // Si no se puede cargar la imagen de fondo, usar color de fondo
                    d.clear_background(menu.background_color);
                }
            } else {
                // Si el path está vacío, usar color de fondo
                d.clear_background(menu.background_color);
            }
        } else {
            // Si no hay imagen de fondo, usar color de fondo
            d.clear_background(menu.background_color);
        }
        ...
        for (index, option) in menu.options.iter().enumerate() {
            ...
            // Dibujar imagen de la opción si existe y no está vacía
            if let Some(ref image_path) = option.image_path {
                if !image_path.is_empty() {
                    if let Some(texture) = self.texture_cache.get(image_path) {
                        let image_size = 50;
                        let image_x = center_x - 200;
                        let image_y = y - 10;
                        
                        d.draw_texture_pro(
                            texture,
                            Rectangle::new(0.0, 0.0, texture.width as f32, texture.height as f32),
                            Rectangle::new(image_x as f32, image_y as f32, image_size as f32, image_size as f32),
                            Vector2::new(0.0, 0.0),
                            0.0,
                            Color::WHITE,
                        );
...
pub fn create_main_menu() -> Menu {
    let mut menu = Menu::new("".to_string());
    menu.set_background_image("assets/spookyspn.png".to_string());
    menu.set_colors(
...
pub fn create_level_selector_menu() -> Menu {
    ...
    menu.set_background_image("assets/spookys.png".to_string());
    menu.set_colors(
